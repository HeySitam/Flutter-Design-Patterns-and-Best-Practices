...
class CartCubit extends Cubit<CartState> {
  ...
  CartCubit()
      : super(const CartState(items: {}, totalPrice: 0, totalItems: 0)) {
  ...
  Future<void> loadCart() async {
    try {
      emit(state.copyWith(isProcessing: true)); // #4
      final cartInfo = await _cartModel.cartInfoFuture;
      // TODO: Should actually copy the Map and not just the reference, which we will do at the end of this chapter
      emit(
        state.copyWith(
          items: cartInfo.items,
          totalPrice: cartInfo.totalPrice,
